...
namespace geometry {
  ...
  Circle::Circle(const Point& point, int64_t rad) : center(point), radius(rad) {
  }
  ...
  bool Circle::ContainsPoint(const Point& other) const {
    int64_t other_x = (other.x - center.x) * (other.x - center.x);
    int64_t other_y = (other.y - center.y) * (other.y - center.y);
    return other_x + other_y <= radius * radius;
  }
